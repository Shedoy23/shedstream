...
async def run() -> int:
    ...
    print("\n[6] Лёркер теряет ставку, но не цели дня")
    # Граница, которую я сам чуть не снёс. Строка «квесты только для активных»
    # существовала ДО правки вовлечённости, и смена смысла слова «активный»
    # молча превратила её во второе наказание: минус все квесты на просмотр.
    # За 5-часовой эфир это 3 600💎 вместо 45 000 — падение в 12 раз вместо
    # обещанного вдвое. Ставка зависит от вовлечённости, цели дня — от того,
    # что человек всё-таки был на эфире.
    async with db._connect() as conn:
        cur = await conn.execute(
            "SELECT current_value FROM quests WHERE channel_id=? AND username=? "
            "AND quest_type='watch_time_30'", (CH, "stale"))
        row = await cur.fetchone()
    check(row is not None and int(row[0]) > 0,
          f"у зрителя без взаимодействия квест на просмотр движется "
          f"(прогресс {row[0] if row else 'строки нет'})")

    print("\n[7] Источник дохода записан — на триаже будет что смотреть")
