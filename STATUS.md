...
1. **Закрыть два блокера онбординга** (работа моя, ни от кого не зависит):
   бот не заходит в чат нового стримера до перезапуска бэкенда; проверить
   путь совсем нового канала целиком, а не по частям. До этого приглашать
   тестировщика бессмысленно — он упрётся в них первым.
...
4. Вердикт Twitch по `0.0.2` → выпустить → собрать `0.0.3` (рефакторинг фронта,
   ...
   зрителя).
5. Первый незнакомый тестировщик, TTTR, каждая подсказка — support incident.
6. Исправлять реальные blockers, а не все пожелания.
