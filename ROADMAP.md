...
9. Twitch Extension `0.0.5` находится в Hosted Test; её frontend, cache-bust и
   ZIP-кандидат неизменяемы. Manager, backend и игровые моды продолжаются;
   backend обязан обслуживать одновременно публичную `0.0.1` и замороженную
   `0.0.5`. Следующий frontend-batch начинается только после вердикта Twitch.
...
**Все четыре пункта закрыты 06.09.** По четвёртому оценка «самая крупная работа»
не подтвердилась, и вот почему: для RimWorld «каталог из игры» решает реальную
боль — сборка меняет содержимое игры, и зашитый список гниёт. В Minecraft
ванильные id есть всегда, поэтому тащить в панель весь реестр ATM10 (сотни видов
одной только еды) бессмысленно: список станет непроходимым, а выигрыш нулевым.
Настоящих болей было две, и обе закрыты дёшево:
1. **Список нельзя было править без релиза расширения** — он жил двумя копиями,
   на бэкенде и во фронте под комментарием «MUST stay a subset». Теперь он
   объявлен на бэкенде один раз и приезжает в панель с сервера.
2. **Бэкенд не знает, что есть в сборке стримера** — мод 0.1.3 сверяет каталог с
   реестром предметов своей сборки и присылает отсутствующее (`colony.catalog`);
   конфиг это вычитает. Свой список мод не держит: спрашивает бэкенд, отвечает
   «есть/нет». Молчание мода = торгуем полным каталогом.

Что СОЗНАТЕЛЬНО не сделано: модовые предметы в продажу не добавляются
автоматически. Это уже не совместимость, а баланс — какой предмет из ATM10
уместно выдавать за 200💎, решает владелец, а не реестр. Канал для этого готов.
