...
- **Отдавать тестировщику надо `0.1.0-alpha.3`, не `alpha.2`.** `alpha.2`
  ...
  Не проверено машиной: сам запуск распакованного EXE — это делает владелец.
  Артефакты `alpha.2` пока лежат рядом; удалить их, чтобы не отправить по
  ошибке (как поступили с `alpha.1`), — отдельное решение владельца.
