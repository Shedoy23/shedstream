...
**Last updated:** 2026-08-01 (13 «неподтверждённых» находок внешнего аудита
прогнаны ПО КОДУ: 7 закрыты, 1 частично, 4 открыты — S-08, S-20, S-21 и бывшая
S-18. Починены S-18 (награда за квест терялась при сбое и платилась дважды) и
S-15 (пустой секрет подписывал токены публичной строкой) + хвост S-13
(4 необратимых действия без подтверждения, ФРОНТ). Полный прогон 35/35 exit 0.
**Три правки ждут деплоя на прод.** Готовность к 0.0.2: блокеры — трек B реестра
(0 из 9 групп), UGC-модерация без интерфейса, 4 мод-правки без проверки в игре,
устаревший ZIP + несохранённый архив 0.0.1)

**Правило, подтверждённое дважды 01.08:** утверждение из нашего же .md про
статус («не выкачено», «сломано у зрителей») перед пересказом владельцу
проверять командой. В тот день два таких оказались протухшими: модерация
озвучки числилась невыкаченной, а она на проде побайтово; «цена вассала
расходится в 12 раз» оказалась путаницей ДВУХ действий в самом changelog.
...
### Аудит ПО СПЕКЕ (`AUDIT_SPEC.md`) — отдельный трек, состояние на 31.07
...
| §6 периодические сторожа | ✅ закрыт, найдено: цикл начисления за просмотр умирал от одного исключения; «висящий сезон» починен только в дуэлях |
| §9 действия на чужого | ✅ закрыт, найдена бесплатная выдача любого предмета (`player.equip_item` по `item_id`) |
| §11 след от денег | ✅ закрыт, найдено: кости и крестики платили призы без записи, крестики — ещё и мимо транзакции |
| §4 мультиарендность вне SQL | ✅ закрыт, находок нет: канал в ключе всех словарей процесса |
| §7 состояние, которое стоит денег | ✅ закрыт, находок нет: оплаченное переживает рестарт |
| §10 схема против кода | ✅ закрыт, находок нет: `init_tables` и миграции не расходятся |
| §14 кто ты такой (JWT и роли) | ✅ закрыт, находок нет: имя/канал/роль только из подписанного токена |
| §16 админские и стримерские двери | ✅ закрыт, находок нет: все 106 меняющих состояние маршрутов имеют дверь |
| §15 ядро платформы | ✅ закрыт, находок нет: все 8 касс ядра атомарны; граница валют стала машинной проверкой |
| §17 Boosty | ✅ закрыт, найдено: замена списка платных подписчиков стирала его при негодных данных |
| §12, §13 | не начаты — оба про фронт, замороженный на CDN до следующего ревью |

**Прогон всех тестов — только `python scripts/run-backend-tests.py`.** Поштучно
на этом ПК врёт: восемь тестов падали `UnicodeEncodeError` ещё на импорте
(cp1251-консоль + эмодзи в `config.py`) и не проверяли ничего. Раннер ставит
кодировку, считает таймаут провалом и судит по коду возврата. Сейчас 28/28.
...
### Контракт замороженного клиента (с 12.09)

Панель на CDN Twitch не меняется до следующего ревью, бэкенд деплоится за
минуты — значит сервер обязан оставаться понятным ИМЕННО тому клиенту, что
лежит в Twitch. Держит `tests/test_frozen_client_0_0_5.py`: берёт архив из
`tests/fixtures/frozen_client_0.0.5/` (сверяет SHA-256 и без совпадения не
идёт), распаковывает, исполняет его скрипты в `node:vm` в порядке из его же
`extension.html` и подаёт ответы НАСТОЯЩИХ обработчиков на временной базе.
Проверяет то, что увидит зритель: числа и тексты, написанные в DOM, и тосты.

Практически это значит: **переименовал поле в ответе API — узнаешь об этом в
CI, а не от зрителя.** Прогон 4 секунды, подхватывается `run-backend-tests.py`.
Новый кандидат — новая папка фикстуры и новый тест рядом; старый живёт, пока
его версия лежит на CDN.

Пара к нему — `tests/test_deploy_frontend_freeze.py`: запускает настоящий
`deploy.ps1 -DryRun` и доказывает, что выкат фронта на прод заперт, пока
`$FrontendReviewOpen = $true`, а бэкенд при этом проходит. Снимать замок —
правкой этого флага в скрипте в день вердикта Twitch.
