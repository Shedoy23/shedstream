...
### Воронка онбординга — собрана и прошла первый живой прогон (2026-08-20)

ROADMAP §R4 требовал двадцать событий; не писалось ни одного. Теперь пишутся:
Manager шлёт свои шаги (до входа в Twitch — без токена, иначе не видно тех, кто
отвалился раньше), бэкенд выводит остальные из того, что уже знает, скачивания
считаются из журнала nginx по крону каждые 15 минут. Смотреть — админка,
раздел «🪜 Воронка».

**Manager впервые опубликован по постоянному адресу:**
`https://shedoy23.ru/releases/ShedLink.Manager-0.1.0-alpha.11-win-x64.zip`
(SHA-256 `47672447…6cb45946`). Ссылка версионная; постоянного `latest` нет
сознательно — он означал бы перезапись опубликованного файла.

**Первый живой прогон владельца:** скачал → готовность за 8 мин 52 с. Из них
работа самого Manager — 14 секунд; остальное загрузка Bannerlord (6 мин 23 с) и
пауза до нажатия кнопки. Отсюда оговорка к воротам M1, записанная в
ROADMAP §5.2-бис: TTTR в нынешнем определении меряет скорее игру и терпение,
чем продукт, поэтому рядом считается вторая метрика — та, которой продукт
управляет.

**Что прогон вскрыл и что уже починено:** ступень «выбрал игру» отмечалась
только при переключении в списке, из-за чего воронка рисовала отвал там, где
никто не отваливался. Правка в Manager, попадёт в следующую сборку.

**Единственное звено, ни разу не сработавшее вживую:** `first_viewer_action`.
Покупки зрителей работают и покрыты тестом, но последняя настоящая покупка на
проде была 05.08, то есть после установки хука ни одной не случилось. Отметится
на первой же покупке в эфире — это и есть определение активации из роадмапа.

### Дашборд стримера чинён (2026-08-20)

Две строки чеклиста врали: «мод на связи» читалось из памяти процесса и
обнулялось при каждом перезапуске бэкенда, «мод хоть раз выходил на связь»
считалось по героям Bannerlord независимо от выбранной игры. Обе теперь
спрашивают `module_last_seen` про активный модуль. Первое, что дашборд показал
после починки: на канале выбран RimWorld, а на связь выходит Bannerlord.

### В день вердикта Twitch по 0.0.2 — порядок действий

Собрано, чтобы не выводить заново. Владелец решил (2026-08-20) звать
тестировщика после одобрения `0.0.2`.

1. **Снять заморозку деплоя:** `$FrontendReviewOpen = $false` в
   `scripts/deploy.ps1`. Пока флаг стоит, `deploy.ps1 -Frontend` падает с
   объяснением — это защита, а не поломка.
2. **Нажать Release** в кабинете Twitch (публикация не автоматическая).
3. **Сверить фронт с тегом** `submit/0.0.2` перед выкладкой батча `0.0.3`:
   в рабочем дереве он уже отличается — там рефакторинг «ядро + игровые
   модули» (шаги 1–3) и дизайн-токены.
4. **Собрать и подать `0.0.3`** по `Расширение/docs/TWITCH_UPDATE_RELEASE_PLAYBOOK.md`.
   В него входят: цены мастерской и каравана (живой дефект с 29 мая,
   `DEFERRED.md`), шаг 8 инструкции ревьюеру, зрительские события воронки
   (`extension_activated`, `first_viewer_open`, `first_viewer_join`).
5. **Перед тестировщиком** — прогнать восемь строк матрицы на втором аккаунте
   Windows (`docs/R3_MATRIX_RUNBOOK.md`). Иначе первый же незнакомый человек
   упрётся в то, что можно было найти за вечер.

**Звать тестировщика ДО одобрения `0.0.2` нельзя, и причина не в списке
доступа.** Я предлагал это 20.08, увидев пустой Streamer Allowlist; владелец
поправил, и он прав: в выпущенной `0.0.1` **сломан магазин RimWorld** — одежда,
оружие, импланты и нейротренеры уходили в эндпоинт питомцев из-за коллизии
глобальных имён `shop.js`/`pets.js`. Починено 22.07 (`0a58956`), но в
публичную версию не попало: фикс сделан после подачи, фронт уже был заморожен.
Подтверждено наблюдением у зрителей 01.08, подробности в
`Расширение/docs/CHANGELOG_0.0.2.md`.

Значит незнакомый стример, выбравший сегодня RimWorld, упрётся в мёртвый
магазин, и первое впечатление о продукте будет об этом. Ждать вердикта — верное
решение, а не осторожность.

(Тот же дефект — причина, по которой существует линтер
`check_frontend_global_collisions`: два файла объявляли `_buyItem`, и загруженный
последним молча съедал первый.)
