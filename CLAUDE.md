...
- **Один признак — один смысл.** Перед использованием поля в условии назвать,
  ...
  28.08. Класс всплывал 4×, дважды про деньги; 13.09 — флаг ожидания в
  автопилоте Bannerlord, стенд его не видел, потому что моделировал только
  то, о чём я уже думал.
...
- **Количество в коде движка — только поиском по ВСЕЙ сборке.** Утверждение
  «X читается / вызывается / подписан в N местах» или «единственный побочный
  эффект — Y» делать поиском по полной декомпиляции (`ilspycmd ... -o`), а
  побочные эффекты — с транзитивными вызовами, не по телу метода. Причина:
  12.09 в одной работе дважды — «флаг читается в двух местах» (семь) и
  «подписчиков семь» (восемь); оба раза грепал отдельные классы. Первое
  привело к лишнему снятию флага, которое меняло торговлю и телепорт и жило в
  сейвах. Не искал полностью — так и пиши. → `LESSONS.md`, «Вывод по выборке».
...
5. **Ломать для проверки — только после коммита, восстанавливать из своей
   ...
   того, как проверка успешно сработала. После восстановления — `git diff`
   пуст и сборка без инкремента: `Copy-Item` переносит старую дату, MSBuild не
   пересобирает и гоняет мутанта; имена копий — не CON/PRN/AUX/NUL (14.09).
   → `LESSONS.md`, «Ломать для проверки».
