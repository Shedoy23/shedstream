...
## Сделано в коде (коммиты `ee058ae`, исправления проверки — `4b0aef8`)
...
1. **Разговор в начале встречи.** Полевая встреча открывает меню `encounter_meeting`,
   ...
   **Опасность:** после `MeetingDone` init сам делает `StartBattle()`, если нет
   ...
   `PlayerEncounter.LeaveEncounter = true`. Проверка 13.09 подтвердила: это не
   нейтральный пропуск разговора, а вход в боевую цепочку без проверки враждебности.
2. **Меню `encounter`** (183514-183529, прочитано): `str_order_attack` (отправить
   ...
   `try_to_get_away_continue` (183539-183546; нужно > 8 здоровых бойцов-не-героев),
   `surrender` (185219). Защищающийся в полевом бою уйти не может; при осаде может,
   если он не на стороне осаждающих и снаружи поселения (MapEventHelper 3562,
   поправка проверки 13.09).
   **Политика «всегда отправлять войска» отвергнута проверкой 13.09:** в заведомо
   проигрышном бою «Попытаться уйти» бывает доступно, а «Отправить войска» бывает
   скрыто или выключено (противник на плоту, нет бойцов кроме героя —
   EncounterOrderAttackCondition 547-575). Нажимать только видимое и включённое;
   выбор между боем и отходом нужен явный.
...
* **Общего безопасного «отказаться» нет** (проверка 13.09, с примерами из кода):
  «Критическое решение королевства» закрытием окна не решается, и движок
  продолжает вести к экрану королевства (SV 12066-12068); единственное «OK» при
  возвращении отряда с альтернативного решения задания возвращает войска в партию
  (CS 122702-122718) — закрыть без него значит потерять отряд; «OK» в предложении
  союза принимает союз или создаёт голосование (CS 166798-166805, 167164-167173).
  Нужны обработчики известных окон по смыслу; незнакомое окно — остановка с
  записью, а не молчаливое закрытие или отказ.
...
## Услуги поселения для партии игрока — СДЕЛАНО 14.09 (`e14dcd0`)

Цепочки перечитаны самостоятельно по телам методов и описаны в заголовке
`src/SettlementServices.cs`. Уточнения к разведке ниже: найм сделан путём экрана
игрока (у игрока и NPC разное правило доступного индекса — 2275 против 209518);
SellItemsAction не проверяет остаток продавца — сверх остатка покупатель получает
товар из ничего (95902), мод ограничивает покупку остатком; героев из списка
пленных на продажу мод убирает — без покупателя движок их отпускает (228223).
Проверка 14.09 (`ecbc951`): цена бойца учитывается до `OnUnitRecruited`, а не после,
как в OnDone, — мод ловит исключения подписчиков и продолжает работу, движок нет;
час последнего прохода пишется в сейв (`IDataStore.SyncData`, BehaviorSaveData 11325).

Разведка 13.09 (исходный текст):
