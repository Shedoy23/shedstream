...
### ⚠ Не запланированная зависимость: как эти пятеро вообще установят расширение

Найдено 2026-08-20 при разборе плана. Twitch дословно:
«After review, changing the allowlist requires re-submitting your Extension for
review» — это записано в `Расширение/docs/TWITCH_UPDATE_RELEASE_PLAYBOOK.md`
с 29.07, но в R4 не учтено.

Следствие: **каждый новый стример в Streamer Allowlist = новая подача и полный
цикл ревью.** Пять стримеров по этому пути — пять циклов. R4 в нынешнем виде
опирается на предположение, что участников можно добавить в консоли.

Три выхода, и выбирать надо ДО начала R4:

1. **Testing Account Allowlist.** Дока говорит, что аккаунты оттуда могут
   установить расширение, минуя Streamer Allowlist, но не говорит, требует ли
   правка этого списка повторного ревью. **Не проверено с 29.07.** Проверка —
   несколько кликов в Dev Console, а решает она план целиком.
2. **Сделать расширение публичным** (пустой список). Это тоже подача, и она
   меняет профиль риска: устанавливать смогут все, включая тех, кого мы не
   готовы поддерживать.
3. **Принять цену.** Пять циклов ревью растянут R4 на месяцы; тогда это надо
   написать в плане явно, а не обнаружить на третьем участнике.

До ответа на этот вопрос дата R4 неизвестна, а не «после R3».
...
⚠ **Инструмента для этого пока нет.** Ни одно из перечисленных ниже событий не
