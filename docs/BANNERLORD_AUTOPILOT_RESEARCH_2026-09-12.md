...
* `DoNotMakeNewDecisions` читается **в двух местах**, и это важно:
  `MobilePartyAi.cs:486` — ветка инициативы (погнаться за соседом / убежать),
  и `AiPartyThinkBehavior.cs:49` — **ранний выход из выбора целей целиком**.
  То есть «мягкий тормоз» он только внутри `MobilePartyAi`; на уровне
  думающего слоя он равносилен выключению. Комментарий в нашем моде описывает
  первое и молчит про второе — для автопилота это означает, что флаг обязан
  быть снят, иначе штатный выбор целей не состоится даже после снятия
  барьера 1.
