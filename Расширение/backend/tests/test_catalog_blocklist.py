...
5. Строка, УЖЕ лежащая в каталоге (её положили до запрета), не показывается
   зрителю и не покупается. Это не перестраховка: каталог перезаписывается
   только когда мод пришлёт новый, то есть при следующем запуске игры — на
   проде 06.09 сабля так и лежала в базе после деплоя фильтра.
"""
...
async def main() -> int:
    ...
    try:
        ...
        # Строка, попавшая в каталог ДО запрета. Оба оставшихся пути —
        # отдача каталога и покупка — требуют JWT, поэтому вызвать их здесь
        # нечем: проверяем, что вызов фильтра в них ЕСТЬ. Компромисс назван
        # прямо, потому что структурная проверка слабее поведенческой — но
        # она ловит именно тот регресс, из-за которого сабля осталась
        # покупаемой после первого деплоя (фильтр стоял только на приёме).
        src = (BACKEND / "rimworld.py").read_text(encoding="utf-8")
        serve = src[src.index("async def get_catalog"):src.index("async def buy_item")]
        check("отдача каталога пропускает снятое",
              "is_blocked_def" in serve, "в get_catalog нет вызова фильтра")
        buy = src[src.index("async def buy_item"):]
        buy_head = buy[:buy.index("balance = await db.get_points")]
        check("покупка отклоняет снятое ДО списания",
              "is_blocked_def" in buy_head,
              "проверка появилась после списания или отсутствует")
