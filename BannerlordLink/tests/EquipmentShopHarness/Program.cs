...
class Program
{
    ...
    static void TestHandlers()
    {
        ...
        request["price_gold"] = 400; hero.Gold = 2500;
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "equipment_price_changed" && hero.Gold == 2500 && behavior.Saved.Items.Count == 0, "Old native price cannot buy after the fivefold increase");
        request["price_gold"] = 2000; hero.Gold = 1999;
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "not_enough_gold" && hero.Gold == 1999, "Authoritative live gold check at multiplied price");
        hero.Gold = 2500; request["save_id"] = "old_save";
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "stale_hero_session" && hero.Gold == 2500, "Queued purchase may not cross saves");
        ...
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "stale_equipment_session" && hero.Gold == 2500 && behavior.Saved.Items.Count == 0, "Reloading older same-save rejects previous-runtime action");
        ...
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "in_mission" && hero.Gold == 2500, "No equipment changes in missions");
        Mission.Current = null; behavior.StoreFails = true;
        Act("hero.buy_equipment", request);
        Check(!ActionFeedback.Applied && hero.Gold == 2500 && behavior.Saved.Items.Count == 0, "Persistence failure must compensate actual gold charge");
        behavior.StoreFails = false; behavior.PushFails = true;
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Applied && hero.Gold == 500 && behavior.Saved.Items.Count == 1, "Successful purchase charges five times native value and survives mirror network failure");
    ...
    static int Main()
    {
        try
        {
            ...
            Check(EquipmentShopPolicy.Price(0) == 5 && EquipmentShopPolicy.Price(900) == 4500, "Native game value is multiplied by five with a five-dinar floor");
