...
- Пакет `0.1.0-alpha.2` собран до этих изменений и им не противоречит: он
  несёт свой манифест старого вида и работает. Следующая сборка подхватит
  новый контракт автоматически.
...
Отдельное ограничение на время Twitch Review: новые правки фронта Extension не
начинаем до вердикта по `0.0.2`. Это не блокирует текущий план Manager и другие
части продукта. Заморозка сверена командами 16.08: поданный архив совпал по
SHA-256, фронт после тега не менялся, cache-bust одинаков в обоих шеллах —
таблица в `Расширение/docs/RELEASE_RECORD.md`.

Висит один открытый вопрос по подаче: прогонял ли владелец шаги Verification
1–8 на живом артефакте перед отправкой. Нигде не записано, значит считаем, что
нет. Ревьюер идёт ровно по этому тексту сейчас, поэтому либо вписать дату
прогона, либо пройти шаги и вписать.
