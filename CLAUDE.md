...
- **Multi-tenant scoping is mandatory.** Every query/insert on a tenant table must include `channel_id`. Resolve it via `resolve_channel_id_or_default()` (admin/legacy boundaries) or the JWT context (`require_jwt_user` / `require_jwt_channel` set a ContextVar). Omitting it causes cross-channel data leaks or `NOT NULL` insert failures. **Now lint-gated:** `scripts/lint_consistency.py:check_tenant_scoping` fails CI/pre-commit on a DML query against a `channel_id`-scoped table that has neither `channel_id` nor a globally-unique id/FK filter (the recurring leak class: marriages, TTS, rimworld all shipped unscoped). Genuine exceptions: trailing `# tenant-ok: <reason>` (or `-- tenant-ok` inside the SQL) on the statement for admin cross-channel / lookup-by-global-id; `tenant-lint: skip-file` anywhere in a known-debt file (rimworld.py is parked there until reactivation).
...
- **Charge + effect in ONE transaction.** `add_points`/`remove_points` open their own connection and commit alone — NEVER pair them with a separate state change across a boundary: a crash/DB-error in the window = money debited without effect, or a reward double-awarded. (2026-07-02 audit found this class **7×** — duels/dice prizes, divorce, TTS, promo, streak; see `docs/AUDIT_CORE_MECHANICS_2026-07-02.md`.) Use `Database.add_points_tx(conn,…)` / `remove_points_tx(conn,…)` — they run the credit/debit on the CALLER's `conn`, so it joins the same `BEGIN IMMEDIATE` transaction as the state INSERT/UPDATE → one commit, all-or-nothing. Same rule for TOCTOU: re-check the guard (cooldown/dedup) INSIDE that transaction, not before it.
- **Платное действие обязано детектить тихий no-op ДО ack-true (иначе зритель платит за ноль).** Класс всплыл 3× в shedcolony (2026-07-04): quest_unlock (unlockQuest = слепой List.add, повтор дублирует запись сейва), set_minimum_stock (addMinimumStock молча no-op'ит на кап-лимите склада), add_xp (addXpToSkill молча no-op'ит на скилкапе = min(99, (уровень_ДОМА+1)×10)). Движковый API «сделай X» часто молча ничего не делает (кап/дубль/нет цели/оффлайн), хендлер отдаёт success → списание без эффекта и без рефанда. Правило: у каждого НОВОГО платного действия найти его no-op пути и возвращать на них fail → авто-рефанд. Детект — по НАБЛЮДАЕМОМУ эффекту (before/after публичными геттерами: getLevel+getTotalXP / isStocked / isUnlocked; или «WorkOrder появился» для void-API вроде requestUpgrade), НЕ по копии внутренней формулы движка (сломается при его апдейте).
