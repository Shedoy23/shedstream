...
- **Проверка и действие, которое она разрешает, — в ОДНОЙ транзакции.**
  ...
  **Правило НЕ про деньги, а про любое условие, которое что-то разрешает.**
  ...
  (`tests/test_module_action_races.py`). 12-й — 14.09 в моде, где базы нет:
  цена нанятого бойца шла в сумму после события найма (чужой код), упади оно —
  боец бесплатный. Без транзакций: учёт сразу за наблюдаемым эффектом, до
  любого вызова чужого кода.
...
- **Платное действие обязано детектить тихий no-op ДО ack-true.** Движковый API
  ...
  украшение лога. **Второй признак (22.09): проверка смотрит на СВОЮ ЖЕ запись,
  а не на результат чужого вызова.** `hero.leave_clan` звал ванильный
  `ApplyWithoutSelectedNewLeader` (при отсутствии наследников тот молча выходит),
  писал «OK», сам занулял `hero.Clan` и «проверял» именно его — а клан оставался
  с ссылкой на ушедшего лидера, и игра падала на суточном тике. Спрашивать:
  «что тут записал Я, а что — движок, и на что я смотрю». Класс всплывал 6×.
  → `docs/BANNERLORD_CRASH_2026-09-22.md`, `LESSONS.md`, «Платное действие обязано детектить тихий no-op».
...
- **Один признак — один смысл.** Перед использованием поля в условии назвать,
  ...
  28.08. Класс всплывал 6×, трижды про деньги; 13.09 — флаг ожидания в
  автопилоте Bannerlord, стенд его не видел, потому что моделировал только
  то, о чём я уже думал; 14.09 — там же «активен экран карты» читался как
  «поверх карты ничего нет», а окна поверх карты экран не меняют (нашёл прогон
  в игре: автопилот обслуживал партию под окном события). 25.09 — магазин
  снаряжения: «отряд, где герой сейчас» (`PartyBelongedTo`) читался как «свой
  отряд», и после проигранной обороны перекованная булава ушла в обоз
  стримера (#66, 60 000💎).
...
- **Перед установкой DLL в игру — свести все ветки, которые её трогают.**
  `git branch -a --no-merged HEAD` и `git worktree list`: есть коммиты в
  `BannerlordLink/` или `BannerlordAutopilot/` вне твоей ветки — влить или
  остановиться. Причина: 22.09 установка из ветки без `codex/postbattle-rest`
  молча стёрла из игры три установки того же вечера.
  → `LESSONS.md`, «установка DLL из своей ветки стирает чужую установку».
...
- **Содержимое игры — из игры, не из кода.** Культуры, войска, вещи, фракции,
  законы, их названия — мод читает из игры и отдаёт на сервер, панель только
  показывает. Новый список id вроде `vlandia`/`imperial_recruit` в коде — только
  с запасным путём, если такого нет. Причина: владелец 26.09 хочет, чтобы в
  будущем расширение принимало даже огромные моды (The Old Realms) без
  переписывания; сейчас жёстко прошито немногое — список в DEFERRED «данные из игры».
...
4. **Тест, который не видели красным, не доказывает ничего.** Показать в двух
   ...
   - **Защиту (предел, резерв, кулдаун) доказывать сценарием, где она
     СРАБАТЫВАЕТ, и мутацией, которая её снимает.** В мире теста, где предел не
     нужен, верно любое утверждение о нём — и «соблюдается», и «потерять его
     безопасно». 14.09 так прожили три дыры автопилота: срок прохода терялся при
     загрузке («потребность пересчитается» — но остаток есть ровно тогда, когда
     предел сработал), а пределы найма за проход не срабатывали ни в одном тесте.
     → `LESSONS.md`, «Защита, проверенная там, где она не срабатывает».
