...
- **Проверка и действие, которое она разрешает, — в ОДНОЙ транзакции.**
  ...
  **Правило НЕ про деньги, а про любое условие, которое что-то разрешает.**
  ...
  одной монеты рядом не было. Признак: между `if <проверка>` и записью,
  которую она разрешает, есть `await`, открывающий другое соединение. Класс
  найден 8×. → `LESSONS.md`, «Charge + effect in ONE transaction».
