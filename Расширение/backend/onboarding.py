...
# Скачивание Manager. Его не присылает никто: в момент скачивания приложение
# ещё не запущено, и единственный свидетель — веб-сервер, отдающий файл. До
# 2026-08-20 ступень была недостижима вовсе (Manager нигде не лежал); теперь он
# опубликован, и скачивания собираются из журнала nginx скриптом
# `scripts/ingest-download-events.py`.
#
# ВАЖНО про шкалу: у скачивания НЕТ `installation_id` — он появляется только
# при первом запуске. Поэтому первая ступень считается «сколько раз скачали», а
# не «сколько людей», и складывать её в одну колонку с остальными нельзя.
# Полезна она разницей: скачали столько-то, запустили меньше — вот первый
# отвал, и там стоит неподписанный exe с предупреждением Windows.
DOWNLOAD_EVENT = "manager_downloaded"
