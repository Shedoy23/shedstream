...
**Вопрос, который стоит проверить раньше вердикта:** Streamer Allowlist пуст с
момента подачи `0.0.2`, а по доке Twitch пустой список означает «расширение
может установить любой стример». Если это уже действует для выпущенной `0.0.1`,
то тестировщика можно звать СЕЙЧАС, не дожидаясь одобрения. Проверяется
кликами в Dev Console, стоит недели ожидания.
