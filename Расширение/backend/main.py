...
# Инициализация
# SHEDSTREAM_DB — путь к базе. По умолчанию «viewers.db» рядом с кодом, то есть
# на проде ничего не меняется. Переменная нужна локальной разработке: она даёт
# держать копию базы ВНЕ репозитория. Локальная база рядом с кодом — это ровно
# та конфигурация, которая 2026-06-25 положила прод: локальный журнал (-wal)
# уехал в архив деплоя и затёр прод-журнал. Тогда починили исключением в
# deploy.ps1, но защита осталась держаться на одной строке в одном скрипте.
# Держать базу снаружи — вторая линия, независимая от той строки.
db = Database(os.getenv("SHEDSTREAM_DB", "viewers.db"))
bot = BotCore(db)
