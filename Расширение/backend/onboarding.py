...
# Событие из ROADMAP §R4, которое СЕГОДНЯ СОБРАТЬ НЕЧЕМ. Сам Manager его
# отправить не может: в момент скачивания он ещё не запущен. Считать его можно
# только на стороне сервера, отдающего файл, — а Manager нигде не опубликован,
# он передаётся из рук в руки. Держим отдельным списком, чтобы дыра была видна
# в коде, а не обнаруживалась при разборе цифр.
#
# Что откроет: публикация Manager на shedoy23.ru. Тогда `manager_downloaded`
# считается по журналу nginx или отдающим эндпоинтом. До тех пор воронка
# начинается с `manager_started` — первого запуска, а не скачивания.
#
# Чем это важно: без него нельзя посчитать «скачал, но так и не запустил», а
# это первая ступень отвала — и именно там стоит неподписанный exe с
# предупреждением Windows.
NOT_YET_COLLECTIBLE = frozenset({"manager_downloaded"})
