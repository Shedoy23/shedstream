...
5. **Судить о тесте по КОДУ ВОЗВРАТА, а не по печати.** «Зелёная» печать без
...
6. **Прод-деплой после стрима, не во время.** Идёт эфир — только хотфикс
...
7. **Фича входит — фича выходит.** На просьбу о новой механике спросить, какую
   малоиспользуемую замораживаем. Причина: разросшийся фронт — это и есть вид
   бесконтрольного «да».
8. **Заданию внешнему аудитору НЕЛЬЗЯ писать «это я уже проверил».** Можно:
...
9. **Предлагать месячный аудит — ДВУХ видов, не путать.** Код-аудит читает код;
...
10. **Обновлять `docs/CONTEXT*.md`** после значимого изменения — это передача
   следующей сессии.
11. **НЕ ЗАПИСАНО = ЗАБЫТО.** Записывать в ту же сессию: найденную поломку,
...
12. **Отложенное — в `DEFERRED.md`, в ту же сессию.** Любое «потом / ждём / решим
    по данным / решили не делать» — строкой: причина + что разблокирует. Раздел
    «Решено НЕ делать» не чистится.
13. **Заканчивать сессию сводкой простым языком:** что изменилось, что где
    задеплоено, что владельцу сделать руками. Он не читает диффы — сводка и есть
    интерфейс.
14. **Пост-стрим-триаж предлагать САМ.** Видно по датам файлов, что был эфир —
    предложить разбор логов, не дожидаясь просьбы.
15. **Факт в документе имеет срок годности.** Любое «X ещё НЕ сделан» перед
...
16. **Обновлять `STATUS.md` в конце сессии** — витрина «что сейчас», 10 строк.
17. **Перед стримом — тест-план на 5 минут + preflight.** Одним сообщением список
...
18. **Рискованные бэк-правки — сначала staging** (`deploy.ps1 -Staging`, :8001,
