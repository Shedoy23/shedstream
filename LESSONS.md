...
- Я **не программист**: проекты делаю в соло с Клодом, по наитию. Код и диффы не читаю —
  **твоя сводка и есть интерфейс**. Последствия объясняй в терминах продукта/зрителей/денег,
  а не техжаргоном («зрители потеряют коннект на ~7с», а не «рестарт uvicorn-воркера»).
- Следствие: **инженерная гигиена — твоя зона инициативы.** Тесты, секьюрити, бэкапы,
  лицензии, мониторинг — я сам это не закажу, потому что не знаю, что должен заказать.
  Предлагай и напоминай проактивно; «работает» ≠ «безопасно/устойчиво».
...
- **Думай, но не тормози.** На обратимом/мелком — действуй сам с явными допущениями, не
  спамь вопросами. Останавливайся и спрашивай ТОЛЬКО когда трактовки реально расходятся И
  цена ошибки высока/необратима; тогда давай варианты с рекомендованным по умолчанию, а не
  открытое «что хочешь».
...
- **Push — в конце каждой сессии ПО УМОЛЧАНИЮ** (решение владельца 2026-07-24:
  ...
  **Условия, без которых push не делается** (добавлено 2026-08-23 после
  ...
  - ветка ожидаемая (`git status` показывает upstream), push без `--force`
    ...
    а не давить.
- **Это стримерский ПК.** Игра может быть запущена (держит DLL мода — копия упадёт),
  стрим может быть live (рестарт прода рвёт коннекты зрителей). Если стрим-статус
  неизвестен из контекста — уточни перед рестартом прода / копией мода. Во время
  стрима на прод — только hotfix падающего.
