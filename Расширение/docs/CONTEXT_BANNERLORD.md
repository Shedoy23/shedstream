...
**Last updated:** 2026-07-29 вечер (багрепорт #42 — платный no-op апгрейда
снаряжения; два действия объявлены в манифесте; цена найма вассала переехала на
бэкенд; блокер: красный тест кассы). До этого: 2026-07-28 (записи «авансом»: фантомные вассалы и замок на
заявках о мире закрыты в бэкенде, ветка `fix/release-gate-money`, НЕ выкачено —
ждём вердикта Twitch. До этого: 2026-06-18 class overhaul Фазы 0-1 ПОСТРОЕНЫ+ДЕПЛОЙ — 12 классов
...
кода, и каждый тест/линтер обязательно показан КРАСНЫМ на подложенной регрессии —
иначе он ничего не доказывает (на этом чуть не обжёгся в §8, см. ниже).

- **§1, цена из тела запроса.** `hero.set_class` и `player.equip_item` списывали
  сумму, присланную клиентом. У первого цену ставили в `_charge_execute_enqueue`
  — то есть ПОСЛЕ вычисления и передачи на списание, хотя комментарий там обещал
  «фиксируем на сервере»; у второго присваивание стояло внутри ветки
  `if random_category`. Проба: прислал `price=12345` → списалось 12345.
  Проверены ВСЕ 27 действий из `_ACTIONS_WITH_OWN_PRICING`, «протекали» ровно эти
  два. Тест `test_client_cannot_set_price` парсит список прямо из исходника
  `_enforce_price` — новое действие, забывшее цену, упадёт само.
- **§1, атомарность.** Сбой после списания и до постановки в очередь откатывает
  всё. Падение вызывается честно, без правки кода: несериализуемое поле в данных
  роняет `json.dumps` уже после `UPDATE viewers`.
- **§3, замок на законах.** Две заявки висели с 24.07 (шесть дней): частичный
  индекс `idx_policy_pending_unique` не давал предложить тот же закон в том же
  королевстве снова, а периодического сторожа не существовало — только разовая
  миграция `m96_policy_requests_unstick`. Добавлен
  `_policy_requests_expire_loop` (раз в час, TTL сутки — те же числа, что у мира).
  Деньги намеренно не возвращаю: действия ушли как `acked`, закон мог сработать.
- **§5, потолок пакета.** Число конвертов в `POST /v1/module/*/events` не
  ограничивалось ничем, кроме nginx; rate-limit к этим маршрутам не применяется.
  `MAX_ENVELOPES_PER_BATCH = 200`. Проверено на стенде: 200 → OK, 201 → 413.
- **§5, граница валют держится:** `add_points` в адаптерах всех трёх игр — ноль
  вхождений, увеличения `points` внутри модулей только возвраты уплаченного.
- **§8 — честно НЕ закрыт.** Гонку на кулдаунах не воспроизвёл четырьмя
  способами (asyncio, живой стенд, снятый per-user лок, 10 одновременных
  запросов). Первая версия теста была зелёной и при снятых защитах — то есть
  ничего не доказывала. Записано как открытое, `DEFERRED.md` §C0-quater.

**Класс ошибки, названный за эти два дня (встретился трижды):** защита выглядит
существующей, а её результат не используется. #42 — число заменённых слотов
считалось и уходило только в лог; `BEGIN IMMEDIATE` в возвратах — защищал не то,
что написано; цена — обнулялась после того, как ушла на списание. **Признак:**
увидел в комментарии «фиксируем/проверяем» — найди строку, где результат этой
проверки влияет на исход.

## 2026-07-29 вечер — платный no-op апгрейда снаряжения (#42) + манифест + цена вассала

Ветка `fix/release-gate-money`. **Ничего не выкачено, DLL в игру не скопирована.**

### #42 — самый дорогой баг волны: деньги за ничего

Зритель: «золото списало, снаряжение не обновилось; пересбор работает».
Разобрано по коду, причина найдена и подтверждена:

`UpgradeGearHandler.ApplyUpgrade` вызывает `ApplyGearLoadout(...)`, которая
возвращает **число реально заменённых слотов** — и это число использовалось
**только в строке лога**. Всё остальное шло безусловно:

- `GiveGoldAction` списывал до **1 500 000💰** (T6),
- на бэкенд уходил `hero.gear_tier_changed`, и `_on_gear_tier_changed` писал
  новый тир в БД **без единой проверки** (его докстринг при этом обещает
  «tier обновляется ТОЛЬКО после successful upgrade» — обещание не подкреплено
  ничем в коде мода),
- `HeroProfileBehavior.SetGearTier` обновлял per-save профиль.

**Хуже потери денег — «убегающий тир»:** после no-op тир в базе всё равно рос,
следующая покупка целилась на ступень выше и повторяла то же самое.

**Фикс:** `slotsFilled <= 0` → `ActionFeedback.PostFailed(actionId,
"gear_no_change")` ДО списания. Деньги и тир не трогаются, зритель получает
объяснение (M103, новая строка в `refusals.py`).

**Почему НЕ трогал подбор предметов.** Гипотеза о первопричине нуля замен:
коммит `9c147b0` (20.07, «квантильные тиры брони») ввёл `PickQuantileArmor` —
броня выбирается по перцентилю очков защиты, а не по движковому
`ItemObject.Tier`. Защита `ShouldReplaceSlot` (`UpgradeGearHandler.cs:275`)
осталась сравнивать **движковый** тир надетого предмета с целевым и блокирует
замену, если надетое «выше тиром». После расцепления одно с другим не связано.
**Это логический вывод из двух фактов, не наблюдение:** в логе 27.07 записей
`upgrade_gear` нет вовсе, в остальные дни `slots filled` всегда 4-10, нулевого
случая поймать не удалось. `ShouldReplaceSlot` бережёт турнирные призы и крафт
— править её вслепую опаснее, чем ждать факта. Подтвердит гипотезу появление
`REFUSE … 0 слотов заменено` в логе после выката.

### Два живых действия работали мимо манифеста

`hero.reforge_quality` и `hero.set_combat_stance`: кнопка есть, цена есть,
хендлер в моде есть — а `supports_action()` отвечал `False`, потому что
`routes/bannerlord.py` пишет их в `module_actions` напрямую, минуя
`dispatch_action`. Сегодня безвредно, на переходе к generic dispatch сломались
бы молча. Добавлены в `extensions.actions` (рядом с блоком Sprint 5.31, где
этот же класс фиксировали для 4 других действий). **Действий стало 68** (7 + 61).

### Цена найма вассала — с бэкенда

`hero.recruit_vassal_clan` показывал зашитые «3 000 000💰» в трёх местах, хотя
бэкенд цену уже отдавал (`hero_gold_costs.recruit_vassal`). Путаница возникла
из-за соседнего `create_vassal_clan` (250K, выделить наследника), который
гидрировали 29.07 — «вассал-кланы сделаны» прочиталось как оба. **Урок:
проверять не механику, а каждое ДЕЙСТВИЕ поимённо.**

### 🔴 Блокер: тест кассы красный

`tests/test_bannerlord_buy_action.py` — 37 PASS / 17 FAIL. Не регрессия:
характеризовал кассу на `player.respawn`, убранном из продажи 29.07 днём.
Замена не механическая — платных действий, не требующих героя, не осталось.
Разбор и варианты: `DEFERRED.md` §C0. **Красный тест блокирует выкат.**

## 2026-07-28 — Записи «авансом»: вассалы и заявки на мир (бэкенд, НЕ выкачено)

Ветка `fix/release-gate-money`, коммиты `bfb203a` + `1b23db5`. Мода не касается.

**Общий класс, ради которого стоит читать этот раздел.** Обе механики писали
строку в базу ДО того, как мод сходит в игру, и рассчитывали закрыть её потом.
«Потом» не наступало — и запись начинала жить как настоящая сущность:

- заготовка вассала (`vassal_clan_id = "pending_<action_id>"`) показывалась
  зрителю, держала наследника занятым и съедала слот из пяти;
- заявка на мир висела в `pending`, а поскольку индекс
  `idx_peace_pending_unique` **частичный** (`WHERE status='pending'`), она
  запрещала тому же королю предложить мир той же фракции ПОВТОРНО — навсегда,
  после одной неудачи. Это была не грязь в таблице, а замок на механике.

**Как закрыто.** Отказ действия (`action.failed`) теперь сносит заготовку
вассала и переводит заявку в терминальный статус — в одном месте,
`_adapter._drop_placeholder_rows`. TTL-сторож зависших действий ходит ТЕМ ЖЕ
путём, поэтому потерянное событие (опасение S-08) не оставляет мусора.
У вассала отдельная колонка не понадобилась: заготовка несёт `action_id` в
своём же id. Заявке колонку добавили миграцией **M101** + суточный сторож для
строк, созданных до неё.

**Разведены два разных вопроса к записи** — это главное, что легко сломать при
правках: «показать зрителю?» (только подтверждённые) и «занимает слот?»
(подтверждённые ПЛЮС заготовки моложе 10 минут). Если уравнять их и
игнорировать все `pending`, вернётся двойное создание по двойному клику.

**Что НЕ сделано и почему:** lookup наследника в моде (4 падения из 4 — там),
событие об успешно заключённом мире, заявки на законы. Всё в `DEFERRED.md`
§A0-nonies. Тесты: `test_vassal_placeholder_lifecycle.py`,
`test_peace_offer_lifecycle.py` — оба видели красными до фикса.

## 🟠 БАГ 2026-07-24 — оплата за вклад считает ТОЛЬКО киллы, урон/поглощение = 0 (триаж лога)

Найдено триажем лога 24.07 (read-only во время стрима). **149 из 149 боёв за
день: `урон=0 поглощено=0`** у ВСЕХ зрителей, даже с 23–31 киллом. Очки = киллы×10,
т.е. оплата за вклад откатилась к «только за добивание» — ровно то, что фикс
2026-07-21 (`bb65d86`) должен был исправить для лучников/латников.

**Корень (виден по коду `DamageHookPatch.cs:~140`):**
```csharp
int dealt = b.InflictedDamage;
dealt = Math.Min(dealt, Math.Max(0, (int)victim.Health));  // обрезка перебоя
NoteDamageDealt(attackerUser, dealt);
```
Обрезка перебоя (удар 200 по врагу с 40 HP = вклад 40) читает `victim.Health`
УЖЕ ПОСЛЕ применения удара — Harmony-prefix на `RegisterBlow` срабатывает, когда
движок уже вычел урон. У добивающего удара `Health`≈0 → `dealt=min(урон,0)=0` →
урон не засчитывается. Обрезка, задуманная резать излишек, обнулила ВЕСЬ урон.
Мой же вчерашний класс ошибки: «починил одно, сломал другое той же правкой».

Поглощение (`NoteDamageAbsorbed`, victimUser) — тоже 0, проверить отдельно
(там клампа нет, значит причина иная — возможно, условие
`victimUser != null && attackerUser == null` почти не выполняется: урон по
зрителю идёт от вражеского NPC, attackerUser=null → должно бы срабатывать;
разобрать при фиксе).

**Фикс (после стрима):** обрезку перебоя делать не по `victim.Health` (пост-хит),
а иначе — например, засчитывать `b.InflictedDamage` целиком, а перебой резать
только на КИЛЛ-блоу через отдельный сигнал (агент умер → вклад = его pre-hit HP).
Или снять клапан вовсе (инфляция от перебоя — меньшее зло, чем нулевой урон).
Severity: средняя (деньги идут, но не тем классам; лучник/латник недополучают).
DLL-правка → деплой после стрима, игру перезапустить.

## 🔴 ИНЦИДЕНТ 2026-07-24 — 3 зрителя погибли одновременно при роспуске королевства (РАЗБОР ПОСЛЕ СТРИМА)

Владелец сообщил на стриме: клан и персонаж slopkom «погибли при неизвестных
обстоятельствах». Разбор по логу мода (`bannerlordlink_20260724.txt`), read-only,
стрим не тронут.

**Факты (точная секунда):**
```
16:32:17.008  OnDailyTick — дневной тик карты
16:32:17.603  мод получил hero.leave_kingdom (slopkom, правитель, «распустить королевство»)
16:32:17.642  [tournament] hero killed → removed 1 from queue  ×3
16:32:17.644  HeroKilled: @slopkom by unknown (Lost) + 4 наследника (heir M2 died)
16:32:17.650  HeroKilled: @dorongh1 by unknown (Lost)
16:32:17.651  HeroKilled: @fikoos418 by unknown (Lost)
16:32:17.676  leave_kingdom завершился: «РАСПУЩЕНО королевство»
```
- Смерти произошли ВНУТРИ обработки leave_kingdom (между .603 и .676).
- Все трое погибших были В ТУРНИРНОЙ ОЧЕРЕДИ — общий знаменатель.
- Единичное событие: за весь день только эти 3 Lost-смерти, больше нет.
- Все трое ПЕРЕРОДИЛИСЬ штатно (система наследника: fresh L18, +1500 gold).
  Перманентной потери нет — slopkom/dorongh1/fikoos418 живы, начали заново.
  Клан/королевство slopkom исчезли как следствие гибели правителя + всей линии.

**Гипотеза (НЕ доказана — не гадать, докопаться):** роспуск королевства
правителем, у которого (или у зрителей его королевства) идёт турнир в городе
этого королевства, каскадно помечает героев `KillCharacterActionDetail.Lost`.
Корреляция очень плотная (один 30-мс интервал, посреди действия) + турнир как
общий знаменатель. Но не ясно, наш ли это каскад или ванильный edge-case.

**Разбор после стрима (по правилу RUNBOOK «декомпиль, а не глотать»):**
1. декомпилировать ванильный путь роспуска королевства (`Kingdom`/
   `DestroyKingdomAction`/`ChangeKingdomAction`) — где героев метит `Lost`;
2. проверить стык с турниром (`TournamentGame`/участники в городе королевства);
3. ванильное → защита в `hero.leave_kingdom` handler (вывести героев из турнира
   ДО роспуска, или не трогать чужих героев); наш каскад → прицельный фикс.
Severity: высокая (viewer-действие → перманентная гибель ДРУГИХ зрителей), но
НЕ пожар: разовое, уже случилось, не повторяется, текущие герои живы.
