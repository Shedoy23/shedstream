...
- Manager перестал знать, какую игру ищет: поиск в Steam, проверка папки, имя
  ...
  передавать манифест.
- Вторая интеграция — **Bannerlord** (решение владельца 2026-08-16). План и
  замеренный объём: `docs/MANAGER_MULTI_GAME_PLAN.md`. Следующий шаг —
  расширить контракт настроек: у Bannerlord плоский `config.json` вместо XML
  и дополнительно нужен `channel_id`.
