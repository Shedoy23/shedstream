...
- **Но отдавать тестировщику именно `alpha.2` уже не стоит:** он собран до
  починки отказа в правах, то есть в нём строка матрицы «недостаточно прав на
  запись» всё ещё означает закрытие приложения. Это ровно тот сценарий, ради
  которого первого тестировщика и зовут, и второго шанса на первое
  впечатление не будет. Перед передачей — пересобрать пакет.
