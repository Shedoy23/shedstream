...
namespace TaleWorlds.MountAndBlade {
 ...
 public class Agent {
  ...
  public void SetScriptedPosition(ref WorldPosition p,bool b,AIScriptedFrameFlags f){if(ThrowScript || ScriptFailuresRemaining-->0)throw new Exception("script failure");ScriptCalls++;Scripted=p;LastFlags=f;} public void DisableScriptedMovement(){if(ThrowDisable)throw new Exception("disable failure");DisableCalls++;Scripted=null;} public void DisableScriptedCombatMovement(){if(ThrowCombat || ThrowCombatOnce){ThrowCombatOnce=false;throw new Exception("combat failure");}} public void SetAutomaticTargetSelection(bool b){if(ThrowTarget)throw new Exception("target failure");} public void SetTargetFormationIndex(int i){}
 }
}
