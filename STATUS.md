...
**Публикация НЕ выполнена — ждёт явного «да, выкатывай».** И отдельно:
для Minecraft «Проверить готовность» сейчас доказывает живой авторизованный
heartbeat, а не исполнение мода — опубликованный JAR ещё не умеет
`diagnostic_ping`. Пока не пройдена живая проверка с настоящим NeoForge и
MineColonies, Minecraft готовым не объявляем.
...
(`Расширение/docs/FRONTEND_MODULE_ARCH_PLAN.md`), добавлен
`frontend/viewer-actions.js` и подключён в обе оболочки. Cache-bust НЕ трогали
