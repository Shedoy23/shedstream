## 25.09 — личный сундук героя без своего отряда

Без своего отряда снятое/купленное ложится в сундук героя (учёт мода без
слота, 10 мест), с качеством перековки; сундук переходит наследнику. Панель
не менялась: бэкенд показывает сундук как багаж. Подробно:
`docs/BANNERLORD_PERSONAL_STASH_2026-09-25.md`.
...
возрастающая queue_seq, периодический повтор. M128, атомарная замена зеркала;
повторная запись успешна, старые события не стирают ожидающих.
Очередь после отката соответствует загруженному save (backend-only импорт удалён).
Аудит вассалов выявил отсутствие восстановления зеркала и подтверждённого rename.
См. ../../docs/BANNERLORD_TOURNAMENT_VASSALS_2026-09-21.md.

## 21.09 — получение команд после сетевого сбоя при запуске

Старт ActionPoller отвязан от успеха разового ping/handshake. Исправление
собрано, проверено и установлено (SHA-256 20fa69d5…bdc723); живой повтор
ждёт запуска игры. Серверная очередь не
менялась. См. ../../docs/BANNERLORD_POLLER_STARTUP_2026-09-21.md.

## 21.09.2026 — синхронизация героев (production)

Конфликт старого владельца hero_id больше не прерывает полный снимок: замена
идентичностей атомарна, прогресс username сохранён, чужие сейвы отклоняются.
Явный build:null показан как старый герой, не как ожидание подключения игры.
Локальные и серверные тесты прошли. Боевой выкат выполнен 21.09 14:09 +05;
health OK, PID 2553136, хэши совпали. DLL не менялась. Живой повтор: 26 героев, сборки AnalniyShekotyn и
shedoy23 ready=true; у shedoy23 управление ограничено пленом. Подробности: ../../docs/BANNERLORD_HERO_SYNC_2026-09-21.md.

# Bannerlord Module — Context (chat handoff)

## 21.09.2026 — привязка настоящего багажа (выкачено 10:48 +05)

Убран fallback к PartyBelongedToAsPrisoner: он открывал багаж пленителя.
Мод блокирует четыре действия со снаряжением в плену/без багажа; backend
зеркалит явный inventory_state вместе с порядком и идентичностью снимка (M127).
Старые снимки без нового состояния не дают доступ к багажу. Личные слоты
видны отдельно; legacy-вещи сохранены и подписаны отдельно. Неизвестный
инвентарь не означает ноль вещей. Отправитель учитывает persistent HeroIdentity.
16+48 C# проверок, backend inventory/equipment/build и два UI-набора прошли;
Release 0 warnings/errors. По подтверждению владельца выполнен staging,
выкачены M127/backend/frontend и установлена DLL с SHA-256 `68b7de60…`.
Health, авторизованный запрос инвентаря и публичные файлы проверены.
Twitch ZIP/CDN не обновлялись. Ожидается живой повтор после загрузки сохранения.
Подробности: `../../docs/BANNERLORD_NATIVE_INVENTORY_2026-09-21.md`.

## 17.09.2026 — магазин и инвентарь за динары (локально)

В нижнюю вкладку «Инвентарь» добавлены «Магазин»/«Мои вещи», поиск, категории,
тиры и слоты. Каталог берётся из запущенной игры. IV открывается на 25-м уровне
героя, V на 30-м; покупка за Hero.Gold по цене `5 × max(1, ItemObject.Value)`
(игровой множитель от 17.09), затем отдельное надевание/снятие.
Владение хранит мод в сейве, backend M125 — зеркало с идентичностью загрузки.
Старые class/upgrade/reequip действия блокируются для сессий нового мода.
Для новых героев: build.version=1, одна специализация, одна оружейная активка,
ранги навыка 50/150, общий оружейный кулдаун 90с, отдельное лечение. Стартовые
боевые навыки 25; один детерминированный бесплатный набор. Класс не задаёт
бонусы и формацию: бонус даёт специализация, формацию — надетое снаряжение.
M126 хранит зеркало build рядом с инвентарём. Старых героев не переносим:
владелец начнёт новую кампанию. Выбор специализации и набора — в «Герое»,
выбор/активация силы — в «Бою», магазин остаётся в нижнем «Инвентаре».
Владелец разрешил frontend-изменения и подтвердил, что Review не отправлялся.
17.09 по запросу владельца backend и frontend /dev опубликованы: миграции M125/M126,
health и публичные хэши проверены. DLL установлена 17.09 01:38 +05 и обновлена
в 01:47 +05 для цены ×5; обе прошлые версии сохранены с хэшами. ZIP/CDN не
менялись; новая цена появится на сервере после запуска игры и каталога.
Живой бой не проверен.
Выкат: `../../docs/BANNERLORD_FREE_BUILDS_DEPLOY_2026-09-17.md`.
Подробности и тест-план: `../../docs/BANNERLORD_EQUIPMENT_SHOP_2026-09-17.md`,
...
Состояние: **работает в игре, цикл замкнут** (прогон 12.09 22:18, лог
`BannerlordAutopilot/evidence/third-run-2026-09-12.log`). Партия трижды подряд
сама прошла «выбрал цель → доехал → вошёл в поселение → штатно вышел → выбрал
заново» (Корсия, Замок Устокол, Жемянь). F12 возвращает управление и чужие
флаги. Барьер выхода из поселения закрыт своим вызовом
`PlayerEncounter.LeaveEncounter` — движок для партии игрока этого не делает.

**Независимая проверка 12.09** нашла восемь групп дефектов, все исправлены (`8e898c8`): бой у поселения принимался за прибытие и закрывался модом, наблюдение меняло состояние, флаги AI оставались изменёнными в сейвах, контракт принимал несовместимый API. Мод теперь **не трогает флаги AI вообще**, выход повторяет кнопку «Уйти» только из мирного поселения. Отчёт и ответ — `BannerlordAutopilot/review/`. **13.09 проверена в игре:** подтверждены контракт 46/46, честный журнал, выход и отмена маршрута при загрузке; прогон нашёл и закрыл ещё три дефекта — пауза после выхода, пинг-понг у ворот, ложное сообщение F12. В игре НЕ проверена встреча с осаждающей партией. Открытый вопрос модели: NPC остаётся в поселении, пока цель — это поселение, автопилот так не умеет.

**13.09, цель владельца «игра играет сама» (ShedLink не трогаем).** Сделано в
коде, в игре НЕ запускалось (`ee058ae`, исправления независимой проверки
`4b0aef8`, `e71560c`): в поселении автопилот нажимает «Подождать» и уходит, когда
пересчёт штатного AI выбрал другую цель; меню трогает только на активной карте;
на карте снимает паузу; осада/рейд/армия не выключают его — берётся лучшее
выполнимое решение (это **своё правило выбора поверх штатных оценок**, не копия
NPC); простой времени и многосуточное пребывание пишутся в журнал. **14.09 —
обслуживание партии в поселении** (`e14dcd0`, в игре не проверено): продажа
обычных пленных, еда и найм штатными расчётами и действиями движка с резервом
денег и потолками трат (README мода, «Обслуживание партии в поселении»). Проверка
14.09 нашла два пропуска, исправлены `ecbc951`: упавшее событие найма оставляло
бойца бесплатным; загрузка сейва обнуляла срок прохода (отметки теперь в сейве).
Стенд 112/0, контракт 177/177. **14.09 с 02:16 эта версия в игре два часа играла
сама:** 10 заездов и выходов, 21 проход обслуживания, 9 нанятых, выключений нет; одна
остановка 161 с в деревне — окно случайного события заперло время, автопилот его не
видел и действовал под ним. Исправлено `5f31d82` (стоит в игре, ещё не запускалось):
под 13 окнами поверх карты автопилот не действует и называет окно; вариант события
выбирает человек. Еда, пленные и пределы трат прогоном не проверены. На бандитов не
нападает и от сильных не убегает: движок выключает эту реакцию для партии игрока. Бой, плен, окна и решения королевства пока не сделаны; план с
поправками проверки — `BannerlordAutopilot/review/unattended-play-engine-notes.md`,
ответы — `BannerlordAutopilot/review/RESPONSE_2026-09-13.md`, `RESPONSE_2026-09-14.md`.
Инструкция — `BannerlordAutopilot/README.md`.
...
## Состояние на 26.09.2026 (коротко; подробности — DEFERRED.md, разделы 25–26.09)

- Выплата за бой (`BattlePayoutPolicy`): формула не менялась; сверху добивка до
  30 000💰 каждому живому участнику в любом бою (`WithFloor`, решение владельца
  26.09, возражение про +46% денег записано); мёртвому герою выплаты нет.
- Автопилот: осада только при своей силе >= 1,5x защитников (было 2x до 26.09) и среднем тире
  бойцов >= 3,5, иначе охота; начатая погоня держится до 6 игровых часов; раз в
  сутки пишет, почему нет крепости по силам.
- Защиты от скрытых правил ванили для зрителей: не исчезает странник без клана,
  клан без королевства не распускается, NPC не женит зрителей, смена правителя не
  переодевает; сундук героя 10 мест для всех; замок стороны в бою.
