...
1. Прогнать восемь строк матрицы R3 на втором аккаунте Windows
   (`docs/R3_MATRIX_RUNBOOK.md`). Единственное, что можно сделать до вердикта.
2. Дождаться вердикта Twitch по `0.0.2`, выпустить, собрать `0.0.3`.
3. Позвать первого незнакомого тестировщика, измерить TTTR, записать каждую
   подсказку как support incident.
4. Исправлять реальные blockers, а не все пожелания.
