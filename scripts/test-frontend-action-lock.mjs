...
// Помощник может быть сколь угодно правильным — важно, что ключ замка его
// РЕАЛЬНО использует. Первая версия этого теста проверяла только помощника и
// молча прошла, когда я вернул старый ключ game+actionType: проверка смотрела
// не туда, куда указывал сам дефект.
const keyLine = (src.match(/var key = [^;]+;/) || [''])[0];
check('ключ замка собирается с параметрами действия',
      /_lockSuffix\(\s*data\s*\)/.test(keyLine), keyLine || 'строка ключа не найдена');
