...
class Program
{
    ...
    static void TestHandlers()
    {
        ...
        request["price_gold"] = 400; hero.Gold = 399;
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "not_enough_gold" && hero.Gold == 399, "Authoritative live gold check");
        hero.Gold = 1000; request["save_id"] = "old_save";
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "stale_hero_session" && hero.Gold == 1000, "Queued purchase may not cross saves");
        ...
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "stale_equipment_session" && hero.Gold == 1000 && behavior.Saved.Items.Count == 0, "Reloading older same-save rejects previous-runtime action");
        ...
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Error == "in_mission" && hero.Gold == 1000, "No equipment changes in missions");
        Mission.Current = null; behavior.StoreFails = true;
        Act("hero.buy_equipment", request);
        Check(!ActionFeedback.Applied && hero.Gold == 1000 && behavior.Saved.Items.Count == 0, "Persistence failure must compensate actual gold charge");
        behavior.StoreFails = false; behavior.PushFails = true;
        Act("hero.buy_equipment", request);
        Check(ActionFeedback.Applied && hero.Gold == 600 && behavior.Saved.Items.Count == 1, "Successful purchase survives mirror network failure");
        ...
        Act("hero.equip_owned", request);
        Check(ActionFeedback.Applied && hero.BattleEquipment[EquipmentIndex.Body].Item == item && hero.Gold == 600, "Owned equip is free and applies selected object");
    ...
    static int Main()
    {
        try
        {
            ...
            Check(EquipmentShopPolicy.Price(0) == 1 && EquipmentShopPolicy.Price(900) == 900, "Engine price floor");
