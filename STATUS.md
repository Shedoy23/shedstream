...
- **Блокер публикации, найден 16.08: PowerShell 7 на этом ПК не установлен.**
  Оба упаковщика мода требуют его и намеренно отказываются работать под 5.1,
  поэтому собрать релизный архив сегодня нельзя ни для RimLink, ни для
  Bannerlord. Ставится одной командой, см. `RUNBOOK.md` §«Публикация в Steam
  Workshop». Существующий `RimLink-0.1.1.zip` при этом цел и совпадает с
  манифестом — блокирована пересборка, а не то, что уже опубликовано.
