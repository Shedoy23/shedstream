...
**Status:** M4.0-M4.5 закоммичены (2026-05-08). Прод НЕ задеплоен — нужен scp с ветки `multitenant`. M5/M6 пенлинг.
...
| 2026-05-08 | M4.0 | `9eb6ef4` | Channels registry. CREATE TABLE channels (channel_id PK, login, display_name, tier, active_module, oauth_*, eventsub_subscription_id). `migrations/m4_channels.py` идемпотентная миграция с backfill из .env. database.py: `get_channel`/`list_channels`/`upsert_channel`. main.py: hook в `run_migrations()` после M1. |
| 2026-05-08 | M4.1 | `8ab07bb` | Eager registration check. `_registered_channels_cache: set[int]` in-memory, sync-проверяется в `require_jwt_user`/`_channel`. `init_registered_channels_cache(db)` вызывается в `on_startup` после миграций. `mark_channel_registered()` — hook для M4.3 OAuth callback'а. JWT с незарегистрированным channel_id → HTTPException(403, detail={status:'channel_not_registered'}). 2 файла, +77 строк. |
| 2026-05-08 | M4.2 (partial) | `df95a6e` | Discoverability рефактор без semantic change: `resolve_channel_id_or_default` — явное имя для legacy soft-fallback на DEFAULT_CHANNEL_ID. 9 callsites (mod endpoints, IRC handlers, admin/craft без JWT, drop_loop crash-recovery, EventSub defensive) переключены на новое имя. Strict-mode для resolve_channel_id отложен — требует threading channel_id через ~5 background paths (event_manager.end_event refunds, duel.check_season_end startup, IRC USERNOTICE streak rewards, casino_bet legacy). 6 файлов, +56/-27. |
| 2026-05-08 | M4.3 | `b684782` | Twitch OAuth flow для регистрации. routes/streamer.py: GET /streamer (landing) → GET /api/streamer/auth/start (CSRF state + 302 на Twitch) → GET /api/streamer/auth/callback (code → tokens, Helix /users, db.upsert_channel + mark_channel_registered, 302 на dashboard). config.py: TWITCH_OAUTH_REDIRECT_URI, TWITCH_OAUTH_SCOPES (`user:read:email channel:read:redemptions`). database.py: upsert_channel расширен OAuth-полями (COALESCE — не затирает). 4 файла, +315/-7. |
| 2026-05-08 | M4.4 | `1f8c88b` | Admin-UI lite — signed session cookies + dashboard. HMAC-SHA256(`channel_id\|expires_at`) с TWITCH_EXTENSION_SECRET, TTL 30 дней, HttpOnly+Secure+SameSite=Lax. Эндпоинты: `/streamer/dashboard` (inline HTML с tier/модуль/login/registered_at/OAuth-статус), `/api/streamer/me` (JSON без OAuth-токенов), `POST /streamer/logout`. /streamer landing редиректит на dashboard если cookie валиден. OAuth callback теперь set cookie + 302 на dashboard. 1 файл, +179/-47. |
| 2026-05-08 | M4.5 | `137c06c` | EventSub auto-register multi-channel за feature flag. `EVENTSUB_AUTO_REGISTER` (default false) → при true iterate db.list_channels() и регистрируем подписку channel.channel_points_*.add для каждого. Per-broadcaster логика вынесена в `_register_eventsub_for_broadcaster` (идемпотентная: GET existing → skip enabled → POST new). Webhook handler не менялся (broadcaster_user_id из payload уже scoped через M3.0). IRC bot per-channel рерайт остался future work (~3-5ч). 2 файла, +93/-43. |
| | M5 | — | Не начат. Per-channel rate limits + tier-based квоты. |
| | M6 | — | Не начат. Testing 2 каналов + deploy. |
| | M4 follow-up | — | (а) Strict resolve_channel_id cleanup — threading channel_id через 5 background paths. (б) IRC bot multi-channel join + scoping. (в) OAuth refresh_token flow для long-lived sessions. ~6-10ч. |
