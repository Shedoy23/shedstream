...
# ── ЗАМОРОЗКА 2026-09-01 ──────────────────────────────────────────────────────
# Кубики сняты с витрины: их место в продукте занял «Перетягивание каната»
# (`routes/tugofwar.py`). Причина — правило Twitch 3.5: в кубиках случайность
# неустранима, это буквально бросок, и именно они стоят первыми в претензии.
#
# Заморозка, а не удаление: сезонная таблица и история матчей остаются
# читаемыми, уже начатые комнаты дорешиваются. Закрыт ровно ОДИН вход — старт
# новой партии. Закрывать надо на бэкенде, а не только снятием кнопки: кнопки
# во фронте нет, но адрес открыт, и это тот же класс, что дыра с атрибутами
# (27.07) и с «Случайным товаром» (01.09).
FROZEN = True
FROZEN_MESSAGE = ("Кубики заморожены — их заменило «Перетягивание каната». "
                  "Сезонная таблица осталась, новые партии не начинаются.")
...
async def check_season_end(channel_id: int = None):
    ...
    async with db._connect() as conn:
        ...
        # 2026-07-31 (выявлено НА ВЫКАТЕ). Ротация создавала новый сезон
        # безусловно — даже когда рядом уже шёл живой, непросроченный. На проде
        # это дало ДВА открытых сезона rps на одном канале: просроченный #2
        # закрылся, за #6 выплатили, и поверх живого #11 родился #12. То есть
        # «висящий сезон», от которого мы и лечились, воспроизводился самим
        # лечением. Плюс сброс `duel_stats` обнулял рейтинги идущего сезона.
        # Живой сезон есть → закрываем просроченные и НЕ трогаем ни его, ни
        # статистику.
        ...
        for rank, (uname, elo) in enumerate(top, 1):
            ...
            if prize:
                ...
                # M105 (журнал заведён 29.07 для дуэлей, сюда не дошёл —
                # аудит спеки §11, 30.07). Без записи выплата не оставляет
                # следа: `duel_stats` обнуляется ЭТОЙ ЖЕ ротацией, и через
                # месяц «кому и сколько заплатили» неразрешимо. Ровно на этом
                # я ошибся в аудите 29.07, заявив «призы не платились никогда»
                # по обнулённой таблице.
                await conn.execute(
        ...
        # Живой сезон уже идёт — новый не заводим и статистику не сбрасываем.
        # Иначе получаем два открытых сезона на одну игру (см. комментарий выше).
        if live:
            await conn.commit()
            return
...
async def dice_play_vs_bot(request: Request):
    ...
    if FROZEN:
        return {"success": False, "message": FROZEN_MESSAGE, "frozen": True}
