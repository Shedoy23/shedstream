...
async def run_migrations():
    ...
    async with aiosqlite.connect(db.db_path) as conn:
        ...
        try:
            from migrations import m111_manager_session_scope
            await m111_manager_session_scope.apply(conn)
        except Exception as e:
            print(f"❌ M111 migration FAILED: {type(e).__name__}: {e}")
            raise

        try:
            from migrations import m112_manager_diagnostics
            await m112_manager_diagnostics.apply(conn)
        except Exception as e:
            print(f"❌ M112 migration FAILED: {type(e).__name__}: {e}")
            raise
