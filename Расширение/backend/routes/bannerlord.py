...
async def bannerlord_my_hero(request: Request):
    ...
    async with db._connect() as conn:
        ...
        # Багрепорт #72 (26.09): замороженная панель перерисовывает блок свиты,
        # только когда меняется JSON списка свиты, а кнопки найма и «не хватает
        # N💰» считает от золота героя на момент отрисовки. Золото после боёв
        # росло, блок — нет, найм был заблокирован до перезагрузки. Это поле —
        # только ключ перерисовки для панели: панель его не читает, но смена
        # золота или предела свиты меняет JSON и блок перерисовывается.
        # Для пустой свиты не срабатывает (нет элементов) — чинить в панели.
        render_key = f"{hero.get('gold') or 0}:{retinue_cap}"
        for member in retinue:
            member["render_key"] = render_key
