...
| Сохранение игры падает | YART (нулевой байт в письме) | риск потерять прогресс эфира |
...
- [ ] **Точка входа — по спросу, а не по алфавиту.** Обработчиков много (только
      в Bannerlord 47 файлов, 60 покупаемых действий), и сплошная проверка
      растянется. Начинать с первой десятки по фактическим покупкам: за вечер
      05.09 это `train_skill` 265, `install_implant` 135, `add_trait` 56,
      `set_passion` 54, `equip_item` 42, `add_gene` 36 — они закрывают больше
      90% покупок. Данные брать запросом к `module_actions` за последние эфиры,
      а не по памяти.
...
- [ ] Проверить, может ли игра сообщить недоступность действия и понятную
      ...
      **Здесь принцип уже нарушен, и это не проверка, а починка:** мод
      подтверждает действие ДО того, как выполнит его (`RUNBOOK.md`, «ACK
      мод-действия не доказывает выполнение»). По объёму этот пункт больше
      остальных вместе взятых — планировать его отдельно.
...
два вопроса: (1) отказывает ли путь ДО списания, когда действие заведомо
неприменимо; (2) подтверждается ли успех наблюдаемым эффектом, а не самим
фактом вызова. Доля ответов «да» и есть метрика; её же считать после правок.

Точка отсчёта, снятая 2026-09-06 (сырые цифры, не выводы):

- Bannerlord: 47 файлов-обработчиков, 41 называет оба исхода; вызовов
  `PostFailed` 333 против `PostApplied` 64 — то есть отказ описан подробно, а
  успех чаще подразумевается.
