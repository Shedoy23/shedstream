# STATUS на 2026-08-24 (архивный снимок)

Снят при сокращении `STATUS.md` до витрины на 10 строк. Здесь файл целиком
в том виде, в каком он разросся до 448 строк: доказанные факты R0-R2,
состояние Manager, история заморозки фронта. Живые части в тот же день
разъехались по домам — эфирные проверки в `RUNBOOK.md`, день вердикта
Twitch в `TWITCH_UPDATE_RELEASE_PLAYBOOK.md`, очередь работ в `ROADMAP.md`.
Ничего не выброшено: то, что не переехало, лежит здесь.
