...
ПРОВЕРЕНО НА STAGING 2026-07-29. Первый прогон удалил только одну из трёх:
`twitch_ids` и `rimworld_catalog` возвращались обратно. Причина — их создаёт
`database.py:init_tables()`, а он вызывается ДВАЖДЫ: до миграций и после.
Миграция дропала, следующий вызов пересоздавал. Поэтому вместе с этой
миграцией из `init_tables()` убраны и сами CREATE — иначе она бесполезна.
Мораль общая: удалять таблицу мало, если её кто-то создаёт при каждом старте.
